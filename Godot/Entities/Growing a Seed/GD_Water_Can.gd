...
func _physics_process(delta: float) -> void:
    ...
    if alignment > 0.95:
        $PickableObject/WATER_CAN_END/GPUParticles3D.amount = 120
    elif alignment > 0.85:
        $PickableObject/WATER_CAN_END/GPUParticles3D.amount = 100
    elif alignment > 0.75:
        $PickableObject/WATER_CAN_END/GPUParticles3D.amount = 90
    elif alignment > 0.7:
